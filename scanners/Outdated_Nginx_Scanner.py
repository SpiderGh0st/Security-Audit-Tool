#!/usr/bin/env python3
"""
Find outdated NGINX servers and list version-applicable CVEs from NVD.

The scanner:
  - fingerprints nginx/x.y.z using Nmap and HTTP Server headers
  - retrieves the current stable version from https://nginx.org/en/download.html
  - queries the NVD CVE API using the exact NGINX CPE/version
...
Examples:
    python3 Outdated_Nginx_Scanner.py -f target_ips.txt
    python3 Outdated_Nginx_Scanner.py 192.0.2.0/24 --show-all
    python3 Outdated_Nginx_Scanner.py -f target_ips.txt --all-ports
    python3 Outdated_Nginx_Scanner.py -f target_ips.txt --nvd-api-key KEY
...
def current_stable(timeout):
    try:
        html = url_text(NGINX_DOWNLOAD, timeout)
        section = re.search(
            r"Stable version(.*?)(?:Legacy versions|Mainline version)",
            html,
            re.I | re.S,
        )
        text = section.group(1) if section else html
        match = re.search(r"nginx-(\d+\.\d+\.\d+)", text, re.I)
        if match:
            return match.group(1), "nginx.org"
...
def extract_nginx(port_node):
    ...
    evidence = "\n".join(values)
    version_match = re.search(r"\bnginx[/\s-]+(\d+(?:\.\d+){1,3})", evidence, re.I)
    nginx_present = bool(re.search(r"\bnginx\b", evidence, re.I))
    return nginx_present, version_match.group(1) if version_match else "", evidence
    ...
    for host_node in root.findall("host"):
        ...
        for port_node in host_node.findall("./ports/port"):
            ...
            if state is None or state.attrib.get("state") != "open":
                continue
            detected, version, evidence = extract_nginx(port_node)
            if not detected:
                continue
            ...
            header = re.search(r"\bnginx(?:[/\s-]+\d+(?:\.\d+){1,3})?", evidence, re.I)
            rows.append({
                "ip": ip,
                "port": port,
                "protocol": protocol,
                "version": version,
                "evidence": header.group(0) if header else "nginx",
            })
    return rows
    ...
    async def worker():
        ...
        while True:
            ...
            try:
                ...
                completed += 1
                sys.stdout.write(
                    f"\rDiscovering NGINX: {completed}/{len(targets)} "
...
def enrich_versions(rows, args):
    ...
    for index, version in enumerate(versions, start=1):
        print(f"Querying NVD {index}/{len(versions)}: nginx {version}", file=sys.stderr)
...
def print_table(rows, colors=True):
    print()
    if not rows:
        print("No outdated or NVD-matched NGINX endpoints were identified.")
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="Find outdated NGINX and version-based CVE candidates from NVD."
    )
    ...
    parser.add_argument("--show-all", action="store_true", help="Show current and version-hidden NGINX too")
    parser.add_argument(
        "--nvd-api-key",
        default=os.environ.get("NVD_API_KEY", ""),
        help="NVD API key; defaults to NVD_API_KEY environment variable",
    )
    parser.add_argument("--nvd-timeout", type=int, default=30, help="NVD/nginx.org request timeout")
    parser.add_argument("--nvd-cache", default="nginx_nvd_cache.json", help="NVD response cache")
    parser.add_argument("--stable-version", help="Override current stable NGINX version")
    parser.add_argument("--csv", default="nginx_vulnerability_results.csv", help="CSV output")
    parser.add_argument("--list", default="nginx_affected_endpoints.txt", help="Affected IP:port list")
    ...
    print(f"NGINX stable   : {stable} ({stable_source})")
    print("CVE source     : NVD CVE API 2.0 using exact nginx CPE/version")
    ...
    print()
    print(f"NGINX endpoints found : {len(discovered)}")
