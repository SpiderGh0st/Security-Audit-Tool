#!/usr/bin/env python3
"""
Check for missing security flags (Secure, HttpOnly) on Cookies.

This scanner:
  - Connects to HTTP/HTTPS services
  - Inspects Set-Cookie headers
  - Reports cookies missing 'Secure' or 'HttpOnly' flags

Requirements:
    sudo apt install nmap

Examples:
    python3 CookieSecurityScanner.py -f target_ips.txt
    python3 CookieSecurityScanner.py 192.0.2.0/24
"""
...
def analyze_cookies(output, protocol):
    results = []
    errors = []
    # Nmap http-headers output format for Set-Cookie
    for line in output.splitlines():
        if "set-cookie" in line.lower():
            match = re.search(r"Set-Cookie\s*:\s*(.+)", line, re.I)
            if match:
                cookie_full = match.group(1).strip()
                cookie_parts = [p.strip() for p in cookie_full.split(";")]
                cookie_name = cookie_parts[0].split("=")[0]
                
                missing = []
                flags = [p.lower() for p in cookie_parts]
                
                if "httponly" not in flags:
                    missing.append("HttpOnly")
                if "secure" not in flags and protocol == "HTTPS":
    ...
    for host_node in root.findall("host"):
        ...
        for port_node in host_node.findall("./ports/port"):
            ...
            if script is not None:
                findings = analyze_cookies(script.attrib.get("output", ""), protocol)
                for f in findings:
                    rows.append({
                        "ip": ip,
                        "port": f"{port}/TCP",
    ...
    async def worker():
        ...
        while True:
            ...
            try:
                ...
                completed += 1
                sys.stdout.write(f"\rScanning cookies: {completed}/{len(targets)} | Findings: {len(results)}".ljust(60))
                sys.stdout.flush()
            finally:
                queue.task_done()
...
def main():
    parser = argparse.ArgumentParser(description="Check for missing cookie security flags.")
    ...
    parser.add_argument("--csv", default="cookie_security.csv", help="CSV output")
    ...
    ports = ",".join(
        dict.fromkeys(
            part.strip()
            for part in ",".join(
                [args.ports, *(str(port) for port in sorted(url_ports))]
            ).split(",")
            if part.strip()
        )
    )
    ...
    results, errors = asyncio.run(
        run_all(targets, workers, ports, args.timeout)
    )

    print("\n" + " ".join(cell(name, width) for name, width in COLS))
    print("-" * 80)
    for row in sorted(results, key=lambda x: ip_sort(x["ip"])):
        print(f"{cell(row['ip'], 16)} {cell(row['port'], 9)} {cell(row['cookie'], 20)} {cell(row['missing'], 30)}")
    if errors:
    ...
    with open(args.csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["ip", "port", "cookie", "missing"])
        writer.writeheader()
